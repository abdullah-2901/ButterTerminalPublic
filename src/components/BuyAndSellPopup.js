...
useEffect(() => {
  ...
  const fetchQuote = async (amount) => {
    ...
    try {
      ...
      if (quoteData && quoteData.outAmount) {
        ...
        let outputAmount;
        if (currentType === 'buy') {
          // Buying: output is token, use token's decimals
    ...
    try {
      ...
      if(currentType=='buy'){
            ...
            requestData = {
              ...
              sol: parseFloat(formData.amount),
              slippage: parseFloat(settings.slippage),
              key: butterWalletCredentials.secretkey,
      ...
      try {
        ...
        console.log(response);
        if (response.status === 200 && response.data.TransID) {
          // Clear the submission toast as we're moving to monitoring
          showToast.dismiss(toastId);
          // Create clickable toast with Solscan link
          const transactionId = response.data.TransID;
          ...
          toast.success(
            ...
            >
              <span>Transaction confirmed! View on Solscan →</span>
            </div>,
            {
          ...
          showToast.dismiss(toastId);
          showToast.error('Failed to submit transaction.');
        }
      } catch (submitError) {
        // console.error('Transaction submission error:', submitError);
        showToast.dismiss(toastId);
        showToast.error('Transaction failed: ' + (submitError.response?.data?.message || submitError.message));
      }
    } catch (error) {
  ...
  return (
      ...
      />
        <div className="bg-[#141414] w-[380px] rounded-2xl p-4 text-white relative">
          ...
          <div className="flex flex-col items-end w-2/3">
              ...
              {isLoadingQuote ? (
                ...
                <div className="text-xl font-bold text-yellow-400">
                  ...
                  {bestRoute && bestRoute.priceImpactPct !== undefined && (
                    <div className="text-xs text-gray-400 mt-1">
                      Price Impact: {(parseFloat(bestRoute.priceImpactPct) * 100).toFixed(2)}%
                    </div>
                  )}
                </div>
              )}
